var JF = {
    initFormat: function (opt) {
        ...
        this.obj = { dateObj: new Date(), regexpObj: new RegExp() };
    ...
    bindEvent: function () {
        $(this.options.dom).find("i").parent("span").unbind("click").bind("click", function () {
            if (JF.isFormated == false) { return; }
            JF.makeContentVisible($(this).next());
        });
        return JF;
    },
    expandAll: function () {
        if (JF.isFormated == false) { return; }
        JF.traverseChildren($(JF.options.dom), function (element) {
            if (element.hasClass('jf-collapsible')) { JF.makeContentVisible(element, true); }
        }, 0);
    },
    collapseAll: function () {
        ...
        JF.traverseChildren($(JF.options.dom), function (element) {
            if (element.hasClass('jf-collapsible')) {
                JF.makeContentVisible(element, false);
            }
        }, 0); return JF;
    },
    ...
    getRow: function (indent, data, isPropertyContent) {
        ...
        if (!isPropertyContent) {
            tabs = JF.multiplyString(indent, JF.tab);
        }
    ...
    formatLiteral: function (literal, quote, comma, indent, isArray, style) {
        ...
        if (isArray) str = JF.getRow(indent, str);
        return str;
    },
    formatFunction: function (indent, obj) {
        var tabs; var i; var str = "";
    ...
    makeContentVisible: function (element) {
        ...
        element.toggle().prev().find('i').toggleClass(this.options.expanded + " " + this.options.collapsed);
        return this;
    },
